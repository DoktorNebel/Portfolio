...
ProductsWindow::ProductsWindow(std::string name, sf::Sprite* sprite, sf::Vector2f position, std::vector<ProductListItem*>* productList, TextureHandler* texHandler, Storage* storage)
{
	...
	this->HoverWindowSprite->setScale(0.3, 0.6);
...
void ProductsWindow::loadButtons(TextureHandler* texHandler, std::vector<ProductListItem*>* productList)
{
	...
	while (iter != end)
	{
		...
		if (!existing)
		{
			this->Buttons.push_back(new TabButton((*iter)->Category, texHandler->GetSprite("SimpleButton")[0][0], this->Sprite->getPosition(), sf::Vector2f(x * 150, y * 50), emptyList));
			this->Buttons.back()->Text.setFont(*texHandler->GetFont());
			...
			{
				this->Buttons.back()->Sprite->scale(1.1, 1);
			}
		...
		while (anotheriter != anotherend)
		{
			if ((*anotheriter)->Name == (*iter)->Category)
			{
				...
				if (!existing)
				{
					...
					TabButton* tmp = new TabButton((*iter)->Subcategory, texHandler->GetSprite("SimpleButton")[0][0], this->Sprite->getPosition(), sf::Vector2f(x * 130, y * 100), emptyList);
					tmp->Text.setFont(*texHandler->GetFont());
				...
				while (yetanotheriter != yetanotherend)
				{
					if ((*yetanotheriter)->Name == (*iter)->Subcategory)
					{
						...
						if (!existing)
						{
							...
							TabButton* tmp = new TabButton((*iter)->Product, texHandler->GetSprite("SimpleButton")[0][0], this->Sprite->getPosition(), sf::Vector2f(x * 130, y * 150), emptyList);
							tmp->Text.setFont(*texHandler->GetFont());
					...
					if ((*yetanotheriter)->Name == (*iter)->Subcategory)
					{
						...
						while (stopwithallthoseiters != stopwithallthoseends)
						{
							if ((*stopwithallthoseiters)->Name == (*iter)->Product)
							{
								...
								InterfaceButton* tmp = new InterfaceButton((*iter)->Name, InterfaceButton::Type::Normal, texHandler->GetSprite((*iter)->Name)[0][0], this->Sprite->getPosition(), sf::Vector2f(x * 100 + 35, y * 250 + 10));
								tmp->Sprite->setScale(1, 1);
								while (tmp->Sprite->getGlobalBounds().height > 100)
								{
									tmp->Sprite->scale(0.9, 0.9);
								}
								tmp->Sprite->setOrigin(tmp->Sprite->getGlobalBounds().width / 2, tmp->Sprite->getGlobalBounds().height / 2);
								tmp->Text.setFont(*texHandler->GetFont());
								tmp->Text.setString((*iter)->Name);
								tmp->Text.setOrigin((int)tmp->Text.getGlobalBounds().width / 2, (int)tmp->Text.getGlobalBounds().height / 2);
								...
								tmp = new Slider((*iter)->Name + "Slider", -0.1, 0.1, Slider::Type::Centered, texHandler->GetSprite("Slider")[0][0], texHandler->GetSprite("SliderSlider")[0][0], this->Sprite->getPosition(), sf::Vector2f(x * 100, y * 350));
								tmp->Sprite->setScale(0.8, 1);
								tmp->Text.setFont(*texHandler->GetFont());
								tmp->Text.setString("0");
								...
								if ((*prodcatiter)->Name == (*iter)->Product)
								{
									...
									while (buttiter != buttend)
									{
										...
										else if ((*buttiter)->Name == (*iter)->Name)
										{
											...
											(*buttiter)->Visible = this->ProductStorage->countProduct((*iter)->Name);
...
void ProductsWindow::update(Input* input, GameData* gameData, std::list<InterfaceMessage>* messages)
{
				...
				if (!dynamic_cast<TabButton*>(buttons.front()))
				{
					...
					while (anotheriter != anotherend)
					{
						if ((*anotheriter)->WasPressed && (*anotheriter)->Name.find("Slider") != std::string::npos)
						{
							...
							this->addToCart(tmp, dynamic_cast<Slider*>(*anotheriter)->Value, gameData);
						}
	...
	this->HoverWindowText.setPosition(input->GetMousePos().x + 20, input->GetMousePos().y + 20);
	this->HoverWindowSprite->setPosition(input->GetMousePos().x + 10, input->GetMousePos().y + 10);
