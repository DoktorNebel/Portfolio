	...
	case Worker::State::Nothing:
		...
		while (iter != end)
		{
			if (this->WorkerType == Worker::Type::Cashier && iter->JobType == StoreJob::Type::Cashiering)
			{
				...
				this->ThePathfinder->RequestPath(sf::Vector2i(this->GridPosition.x + 0.5, this->GridPosition.y + 0.5), sf::Vector2i(this->Object->GridPosition.x + this->Object->WorkPoints[0].x, this->Object->GridPosition.y + this->Object->WorkPoints[0].y), this->ID);
			...
			if (this->WorkerType == Worker::Type::Cleaner && iter->JobType == StoreJob::Type::Cleaning && !iter->Taken)
			{
				...
				sf::Vector2i pos = sf::Vector2i(iter->Object->GridPosition.x, iter->Object->GridPosition.y);
				sf::Vector2i point;
				this->Object = dynamic_cast<StoreObject*>(iter->Object);
				if (this->Object->AccessPoints.size() > 0)
				{
					point = this->Object->AccessPoints[rand() % this->Object->AccessPoints.size()];
					pos = sf::Vector2i(iter->Object->GridPosition.x + point.x, iter->Object->GridPosition.y + point.y);
				}
				this->ThePathfinder->RequestPath(sf::Vector2i(this->GridPosition.x + 0.5, this->GridPosition.y + 0.5), pos, this->ID);
				this->WorkState = Worker::State::WaitingForPath;
				iter->Taken = true;
			...
			if (this->WorkerType == Worker::Type::Storager)
			{
				if (iter->JobType == StoreJob::Type::Refilling)
				{
					...
					sf::Vector2i pos;
					this->Object = dynamic_cast<StoreObject*>(iter->Object);
					for (int i = 0; i < this->Object->Slots.size(); i++)
					{
						if (this->Object->Slots[i] && this->Object->Slots[i]->Name == iter->ProductItem->Name)
						{
							pos = sf::Vector2i(this->Object->GridPosition.x + this->Object->AccessPoints[i].x, this->Object->GridPosition.y + this->Object->AccessPoints[i].y);
						}
					}
					this->ThePathfinder->RequestPath(sf::Vector2i(this->GridPosition.x + 0.5, this->GridPosition.y + 0.5), pos, this->ID);
					this->WorkState = Worker::State::WaitingForPath;
				...
				else if (iter->JobType == StoreJob::Type::Butchering)
				{
					...
					this->ThePathfinder->RequestPath(sf::Vector2i(this->GridPosition.x + 0.5, this->GridPosition.y + 0.5), sf::Vector2i(this->animal->GridPosition.x, this->animal->GridPosition.y), this->ID);
					this->WorkState = Worker::State::WaitingForPath;
					this->ProductItem = iter->ProductItem;
					this->Jobs->erase(iter);
					break;
			...
			for (int i = 0; i < this->Object->Slots.size(); i++)
			{
				if (this->Object->Slots[i] && this->ProductItem->Name == this->Object->Slots[i]->Name)
				{
					if (this->Object->Products[i].size() > 0 && this->Object->Products[i].front().Name != this->ProductItem->Name)
					{
						...
						for (int j = 0; j < this->Object->ShownProducts[i].size(); j++)
						{
