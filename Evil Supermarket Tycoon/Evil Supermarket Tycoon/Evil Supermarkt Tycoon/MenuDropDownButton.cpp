...
MenuDropDownButton::MenuDropDownButton(std::string name, sf::String text, sf::Sprite* sprite, sf::Sprite* listSprite, sf::Vector2f position, std::vector<std::string> elements, TextureHandler* texHandler, sf::Vector2f screenSize)
{
	...
	this->Text.setPosition((int)this->Sprite->getPosition().x - this->Text.getGlobalBounds().width - 10, (int)this->Sprite->getPosition().y + 3);
...
{
	this->MousePos = sf::Vector2f(input->GetMousePos().x, input->GetMousePos().y);
...
void MenuDropDownButton::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
	...
	this->text->setPosition((int)this->Sprite->getPosition().x + 5, (int)this->Sprite->getPosition().y + 2);
	...
	if (this->Pressed)
	{
		for (int i = 0; i < this->Elements.size(); i++)
		{
			...
			this->text->setPosition((int)this->ListSprite->getPosition().x + 5, (int)this->ListSprite->getPosition().y + 2);
			target.draw(*this->text);
		}
	}
}
