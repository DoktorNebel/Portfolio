...
MenuSlider::MenuSlider(std::string name, sf::String text, float min, float max, sf::Sprite* sprite, sf::Sprite* sliderSprite, sf::Vector2f position, TextureHandler* texHandler, sf::Vector2f screenSize)
{
	...
	this->Text.setPosition((int)this->Sprite->getPosition().x - this->Sprite->getGlobalBounds().width / 2 - this->Text.getGlobalBounds().width / 2 - 5, (int)this->Sprite->getPosition().y - 5);
	...
	this->text.setPosition((int)this->Sprite->getPosition().x + this->Text.getGlobalBounds().width + 5, (int)this->Sprite->getPosition().y);
	this->text.setOrigin((int)this->Text.getGlobalBounds().width / 2, (int)this->Text.getGlobalBounds().height / 2);
}
...
void MenuSlider::update(Input* input, std::list<MenuMessage>* messages)
{
	...
	{
		float xPos = input->GetMousePos().x;
...
void MenuSlider::AdjustPos(sf::FloatRect screen)
{
	...
	this->Text.setPosition((int)this->Sprite->getPosition().x - (int)this->Sprite->getGlobalBounds().width / 2 - (int)this->Text.getGlobalBounds().width / 2 - 10, (int)this->Sprite->getPosition().y - 5);
	this->text.setPosition((int)this->Sprite->getPosition().x + (int)this->Sprite->getGlobalBounds().width / 2 + (int)this->Text.getGlobalBounds().width / 2 + 10, (int)this->Sprite->getPosition().y - 5);
}