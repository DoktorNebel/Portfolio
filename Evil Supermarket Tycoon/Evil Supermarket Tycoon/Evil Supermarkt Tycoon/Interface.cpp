...
{
	this->Windows.push_back(new BuildingWindow("Building", texHandler->GetSprite("Window")[0][0], sf::Vector2f(10, window->getSize().y - 300), texHandler));
	this->Windows.push_back(new DataWindow("Data", texHandler->GetSprite("Window")[0][0], sf::Vector2f(100, 10), texHandler));
	this->Windows.push_back(new ProductsWindow("Products", texHandler->GetSprite("Window")[0][0], sf::Vector2f(100, 100), productList, texHandler, storage));
	this->Windows.push_back(new EntityWindow("Entities", texHandler->GetSprite("Window")[0][0], sf::Vector2f(10, window->getSize().y - 300), texHandler, jobs));
	this->Windows.push_back(new WorkersWindow("Workers", texHandler->GetSprite("Window")[0][0], sf::Vector2f(100, 100), texHandler, workers));
	this->Windows.push_back(new HiringWindow("Hiring", texHandler->GetSprite("Window")[0][0], sf::Vector2f(200, 150), texHandler, peepGenerator));
	this->Windows.push_back(new FeedWindow("Feed", texHandler->GetSprite("Window")[0][0], sf::Vector2f(100, 50), texHandler, feed));
	this->Windows.push_back(new IllegalWindow("Illegal", texHandler->GetSprite("Window")[0][0], sf::Vector2f(100, 100), texHandler, illegalStuff));
	this->Windows.push_back(new PenaltyWindow("Penalty", texHandler->GetSprite("Window")[0][0], sf::Vector2f(430, 350), texHandler));
...
void Interface::update(Input* input, GameData* gameData, std::vector<std::vector<std::list<Entity*>>>* grid, std::deque<unsigned int>* freeIDs, unsigned int* highestID)
{
		...
		while (anotheriter != anotherend)
		{
			...
			else if (anotheriter->MessageType == InterfaceMessage::Type::HireWorker)
			{
				if ((*iter)->Name == anotheriter->Text)
				{
					...
					((Entity*)anotheriter->Pointer)->GridPosition = sf::Vector2f(x, y);
					((Entity*)anotheriter->Pointer)->LowestGridPosition = sf::Vector2f(x, y);
	...
	if (!this->BuildMode && input->JustClicked(sf::Mouse::Left) && !this->containsMouse(input))
	{
		...
		for (int i = 0; i < 4; i++)
		{
			if (gridPos.x + i > 0 && gridPos.x + i < (*grid).size() &&
				gridPos.y + i > 0 && gridPos.y + i < (*grid)[0].size())
			{
				...
				while (anotheriter != anotherend)
				{
						...
						dynamic_cast<StoreObject*>(*anotheriter)->ObjectType != StoreObject::Type::Wall))
					{
						...
						if (dynamic_cast<Customer*>(*anotheriter) && dynamic_cast<Customer*>(*anotheriter)->ShoppingState == Customer::ShopState::Stealing)
						{
							gameData->Prestige += 3;
							double money = 0.0;
							for (int i = 0; i < (*anotheriter)->Products.size(); i++)
							{
...
bool Interface::containsMouse(Input* input)
{
	...
	while (iter != end)
	{
		if ((*iter)->isOpen() && (*iter)->Sprite->getGlobalBounds().contains(mousePos.x, mousePos.y))
		{
			return true;
		}
		iter++;
