	...
	while (iter != end)
	{
		...
		if ((*iter)->JustPressed)
		{
			...
			if ((*iter)->Name == "Hire")
			{
				...
				for (int j = 0; j < this->Buttons.size() - this->Workers.size(); j++)
				{
					yetanotheriter++;
				}
				...
				for (int j = this->Buttons.size() - this->Workers.size(); j < this->Buttons.size() + 2; j++)
				{
					yetanotherend--;
				}
...
void HiringWindow::GenerateWorkers()
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		this->Buttons.push_back(new InterfaceButton(nameString, InterfaceButton::Type::Permanent, texHandler->GetSprite(this->Workers.back()->SpriteName + "_Portrait")[0][0], this->Sprite->getPosition(), sf::Vector2f(50, 50 + i * 100)));
		...
		this->Buttons.push_front(new Bar(nameString + "_Cleaning", L"Putzen", this->texHandler->GetSprite("InterfaceBar")[0][0], this->texHandler->GetSprite("InterfaceBarFill")[0][0], this->texHandler, this->Sprite->getPosition(), sf::Vector2f(450, 50 + i * 100)));
		dynamic_cast<Bar*>(this->Buttons.front())->SetValue(this->Workers.back()->CleaningEfficiency);

		this->Buttons.push_front(new Bar(nameString + "_Storage", L"Lager", this->texHandler->GetSprite("InterfaceBar")[0][0], this->texHandler->GetSprite("InterfaceBarFill")[0][0], this->texHandler, this->Sprite->getPosition(), sf::Vector2f(450, 75 + i * 100)));
		dynamic_cast<Bar*>(this->Buttons.front())->SetValue(this->Workers.back()->RefillingEfficiency);

		this->Buttons.push_front(new Bar(nameString + "_Checkout", L"Kasse", this->texHandler->GetSprite("InterfaceBar")[0][0], this->texHandler->GetSprite("InterfaceBarFill")[0][0], this->texHandler, this->Sprite->getPosition(), sf::Vector2f(450, 100 + i * 100)));
