...
void Customer::update(sf::Time elapsedTime, unsigned short gameSpeed, GameData* gameData)
{
		...
		if (this->WaitTimer <= 0.0f)
		{
			if (this->Path.empty())
			{
				...
				case Customer::ShopState::Buying:
					...
					if (checkPos.x >= 0 && checkPos.x < (int)this->Grid->size() &&
						checkPos.y >= 0 && checkPos.y < (int)(*this->Grid)[0].size())
					{
				...
				if (abs(this->GridPosition.x - this->Path.front().x) < 0.1f && abs(this->GridPosition.y - this->Path.front().y) < 0.1f)
				{
					...
					if (!this->Path.empty())
					{
						...
						if (this->Path.size() <= 1 && this->ShoppingState == Customer::ShopState::CheckingOut)
						{
							...
							iter = (*this->Grid)[(unsigned int)(this->GridPosition.x + 0.5f)][(unsigned int)(this->GridPosition.y + 0.5f)].begin();
							end = (*this->Grid)[(unsigned int)(this->GridPosition.x + 0.5f)][(unsigned int)(this->GridPosition.y + 0.5f)].end();
	...
	case Customer::State::Nothing:
		...
		case Customer::ShopState::Buying:
			...
			if (this->NextProduct < (int)this->ShoppingList.size())
			{
				tmp.ID = this->ID;
	...
	case Customer::State::WaitingForPos:
		if (this->Messages->front().MessageType == StoreMessage::Type::SendPos && this->Messages->front().ID == this->ID)
		{
			...
			{
				this->ThePathfinder->RequestPath(sf::Vector2i((int)(this->GridPosition.x + 0.5f), (int)(this->GridPosition.y + 0.5f)), this->Messages->front().Position, this->ID);
	...
	case Customer::State::TakingProduct:
		...
		if (this->WaitTimer <= 0)
		{
			...
			if (this->Object != 0)
			{
				for (unsigned int i = 0; i < this->Object->Products.size(); i++)
				{
	...
	case Customer::State::StandingInLine:
		...
		if (this->WaitTimer <= 0)
		{
			...
			else
			{
				...
				iter = (*this->Grid)[(unsigned int)((int)this->Object->GridPosition.x + this->Object->AccessPoints[this->LinePosition - 1].x)][(unsigned int)((int)this->Object->GridPosition.y + this->Object->AccessPoints[this->LinePosition - 1].y)].begin();
				end = (*this->Grid)[(unsigned int)((int)this->Object->GridPosition.x + this->Object->AccessPoints[this->LinePosition - 1].x)][(unsigned int)((int)this->Object->GridPosition.y + this->Object->AccessPoints[this->LinePosition - 1].y)].end();
				...
				{
					this->Path.push_back(sf::Vector2i((int)this->Object->GridPosition.x + this->Object->AccessPoints[this->LinePosition - 1].x, (int)this->Object->GridPosition.y + this->Object->AccessPoints[this->LinePosition - 1].y));
