...
void WorkersWindow::AddWorker(Worker* worker)
{
	...
	this->Buttons.back()->Sprite->move(50.0f, 50.0f + ((float)this->Workers->size() - 1.0f) * 100.0f);
	...
	this->Buttons.push_back(new InterfaceButton(sstream.str(), InterfaceButton::Type::Normal, this->texHandler->GetSprite("Portrait_Empty")[0][0], this->Sprite->getPosition(), sf::Vector2f(50.0f, 50.0f + 100.0f * (float)this->Workers->size())));
}