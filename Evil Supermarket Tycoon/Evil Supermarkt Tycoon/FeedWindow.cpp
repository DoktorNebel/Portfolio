...
{
	if ((int)this->Feed->size() > this->PrevSize)
	{
	...
	{
		this->PullButton->Sprite->setPosition(this->PullButton->Sprite->getPosition().x, (float)input->GetMousePos().y);
...
void FeedWindow::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
	...
	{
		int opacity = (int)((this->MessageCount / 1.8f - (float)i) * 255.0f);
		if (opacity > 255 || i == 0)
		{
			opacity = 255;
		}
		...
		if (feediter->Pointer != 0)
		{
			(*buttoniter)->Sprite->setPosition(this->PullButton->Sprite->getPosition() + sf::Vector2f(450.0f, -40.0f - 25.0f * (float)i));
		...
		(*textiter)->setPosition(this->PullButton->Sprite->getPosition() + sf::Vector2f(-500.0f, -35.0f - 25.0f * (float)i));
