...
MenuPermanentButton::MenuPermanentButton(std::string name, sf::String text, sf::Sprite* sprite, sf::Vector2f position, TextureHandler* texHandler, sf::Vector2f screenSize)
{
	...
	this->Text.setPosition(round(this->Sprite->getPosition().x - this->Sprite->getGlobalBounds().width / 2.0f - this->Text.getGlobalBounds().width / 2.0f - 5.0f), round(this->Sprite->getPosition().y + 1.0f));
...
void MenuPermanentButton::update(Input* input, std::list<MenuMessage>* messages)
{
	...
	if (this->Sprite->getGlobalBounds().contains((float)MousePos.x, (float)MousePos.y) || this->Text.getGlobalBounds().contains((float)MousePos.x, (float)MousePos.y))
	{
		this->MouseOver = true;
