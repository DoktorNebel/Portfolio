...
void Animal::update(sf::Time elapsedTime, unsigned short gameSpeed, GameData* gameData)
{
	...
	else
	{
		...
		if (this->WaitTimer <= 0.0f)
		{
			...
			while (!pos && j < 4)
			{
				...
				if (i == 0)
				{
					this->NextPos = sf::Vector2i((int)this->GridPosition.x + 1, (int)this->GridPosition.y);
					this->Rotation = 0;
				}
				else if (i == 1)
				{
					this->NextPos = sf::Vector2i((int)this->GridPosition.x, (int)this->GridPosition.y + 1);
					this->Rotation = 1;
				}
				else if (i == 2)
				{
				...
				if (this->NextPos.x > 0 && this->NextPos.x < (int)this->Grid->size() &&
					this->NextPos.y > 0 && this->NextPos.y < (int)(*this->Grid)[0].size())
				{
	...
	float screenPosX = ((this->GridPosition.x - 0.5f) - (this->GridPosition.y + 0.5f)) * Constants::GridSize;
	float screenPosY = ((this->GridPosition.x - 0.5f) + (this->GridPosition.y + 0.5f)) * Constants::GridSize / 2;
	...
	this->LowestScreenPosition = sf::Vector2f(screenPosX - 0.5f * Constants::GridSize, screenPosY - 0.5f * Constants::GridSize);
}
