...
void Game::update(sf::Time elapsedTime, Input* input, Interface* UI)
{
	...
	case Game::State::Intro:
		...
		this->Intro1.setColor(sf::Color(255, 255, 255, (unsigned char)(255 * this->IntroFade)));
		this->Intro2.setColor(sf::Color(255, 255, 255, (unsigned char)(255 * this->IntroFade)));
	...
	if (!this->GameMenu.Messages.empty())
	{
		...
		else if (this->GameMenu.Messages.front().MessageType == MenuMessage::StartGame)
		{
			...
			for (int i = 0; i < this->CurrentStore->GridSizeX; i++)
			{
				for (int j = 0; j < this->CurrentStore->GridSizeY; j++)
				{
					...
					this->CurrentStore->Grid[i][j].back()->GridPosition = sf::Vector2f((float)i, (float)j);
			...
			for (unsigned int i = 0; i < this->PenaltyLevel.size(); i++)
			{
		...
		{
			this->SoundHandler.SetSoundVolume((int)this->GameMenu.Messages.front().Value);
			this->GameMenu.Messages.pop_front();
		}
	}
}
...
void Game::Save(std::wstring path)
{
	...
	for (unsigned int i = 0; i < this->CurrentStore->PeepGenerator.ClassProbabilities.size(); i++)
	{
		...
		for (int x = 0; x < this->CurrentStore->GridSizeX; x++)
		{
			...
			while (iter != end)
			{
				if (!processed[(*iter)->ID])
				{
					...
					for (unsigned int i = 0; i < (*iter)->Products.size(); i++)
					{
					...
					if (object)
					{
						for (unsigned int i = 0; i < object->AccessPoints.size(); i++)
						{
						...
						for (unsigned int i = 0; i < object->Dates.size(); i++)
						{
						...
						for (unsigned int i = 0; i < object->Prices.size(); i++)
						{
							file << "ObjectPrice=" << object->Prices[i] << "\n";
						}
						file << "ProdsPerLayer=" << object->ProdsPerLayer << "\n";
						file << "ProdsPerSlot=" << object->ProdsPerSlot << "\n";
						for (unsigned int i = 0; i < object->ProductPositions.size(); i++)
						{
							for (unsigned int j = 0; j < object->ProductPositions[i].size(); j++)
							{
								for (unsigned int k = 0; k < object->ProductPositions[i][j].size(); k++)
								{
						...
						for (unsigned int i = 0; i < object->ShownProducts.size(); i++)
						{
							for (unsigned int j = 0; j < object->ShownProducts[i].size(); j++)
							{
								file << "ShownProduct" << i << "=" << object->ShownProducts[i][j] << "\n";
							}
						}
						for (unsigned int i = 0; i < object->Slots.size(); i++)
						{
							if (object->Slots[i])
							{
						...
						for (unsigned int i = 0; i < object->WorkPoints.size(); i++)
						{
					...
					if (customer)
					{
						...
						for (unsigned int i = 0; i < customer->PriceExpectancy.size(); i++)
						{
							file << "PriceExpectancy=" << customer->PriceExpectancy[i] << "\n";
						}
						for (unsigned int i = 0; i < customer->ProductsFound.size(); i++)
						{
							file << "ProductFound=" << customer->ProductsFound[i] << "\n";
						}
						for (unsigned int i = 0; i < customer->ShoppingList.size(); i++)
						{
...
void Game::Load(std::wstring path)
{
	...
	while (std::getline(file, line))
	{
		...
		if (field == L"Hygiene")
		{
			this->Data.Hygiene = (float)_wtof(value.c_str());
		}
		if (field == L"Prestige")
		{
		...
		if (field == L"Minute")
		{
			this->Data.Time.Minute = (float)_wtof(value.c_str());
		}
		...
		{
			this->CurrentStore->OpeningDays[day] = _wtoi(value.c_str()) == 1;
			day++;
			if (day == 7)
			{
				day = 0;
		...
		if (field == L"ClassProbability")
		{
			...
			this->CurrentStore->PeepGenerator.ClassProbabilities.push_back((float)_wtof(value.c_str()));
		}
		...
		{
			this->CurrentStore->PeepGenerator.AverageStealingPotential = (float)_wtof(value.c_str());
		}
		if (field == L"SpawnTimer")
		{
			this->CurrentStore->SpawnTimer = (float)_wtof(value.c_str());
		}
		if (field == L"SpawnRate")
		{
			this->CurrentStore->SpawnRate = (float)_wtof(value.c_str());
		}
		if (field == L"Spawn")
		{
			this->CurrentStore->Spawn = _wtoi(value.c_str()) == 1;
		}
		if (field == L"MesID")
		{
		...
		{
			this->CurrentStore->DrugsDelivered = _wtoi(value.c_str()) == 1;
		}
		if (field == L"DrugHour")
		{
		...
		if (field == L"NewEntity")
		{
			if (entity != 0)
			{
				if (dynamic_cast<Animal*>(entity))
				{
					...
					this->CurrentStore->Grid[(unsigned int)entity->GridPosition.x][(unsigned int)entity->GridPosition.y].push_back(entity);
				}
				else if (dynamic_cast<Person*>(entity))
				{
					...
					for (unsigned int i = 0; i < person->Emoticons[0].size(); i++)
					{
				...
				else
				{
					...
					for (int i = 0; i <= abs(object->Width); i++)
					{
						...
						{
							int setPosX = (int)entity->GridPosition.x + i * (1 + (object->Width < 0) * -2);
							int setPosY = (int)entity->GridPosition.y + j * (1 + (object->Height < 0) * -2);
		...
		if (field.find(L"ProductPosition") != std::wstring::npos)
		{
			...
			{
				if ((int)object->ProductPositions.size() <= i)
				{
					object->ProductPositions.resize(i + 1);
				}
				if ((int)object->ProductPositions[i].size() <= j)
				{
		...
		if (field.find(L"ShownProduct") != std::wstring::npos)
		{
			...
			if ((int)object->ShownProducts.size() <= i)
			{
				object->ShownProducts.resize(i + 1);
			}
			object->ShownProducts[i].push_back(_wtoi(value.c_str()) == 1);
		}
		if (field == L"Slot")
		{
			if (value == L"-1")
		...
		if (field == L"Deleted")
		{
			customer->Deleted = _wtoi(value.c_str()) == 1;
		}
		...
		{
			customer->HygieneExpectation = (float)_wtof(value.c_str());
		}
		if (field == L"InLine")
		{
			customer->InLine = _wtoi(value.c_str()) == 1;
		}
		...
		{
			customer->ProductsFound.push_back(_wtoi(value.c_str()) == 1);
		}
		...
		{
			customer->StealingPotential = (float)_wtof(value.c_str());
		}
		if (field == L"Waited")
		{
			customer->Waited = _wtoi(value.c_str()) == 1;
		}
		if (field == L"AtWorkplace")
		{
			worker->AtWorkplace = _wtoi(value.c_str()) == 1;
		}
		if (field == L"CashieringEfficiency")
		{
			worker->CashieringEfficiency = (float)_wtof(value.c_str());
		}
		if (field == L"CleaningEfficiency")
		{
			worker->CleaningEfficiency = (float)_wtof(value.c_str());
		}
		...
		{
			worker->RefillingEfficiency = (float)_wtof(value.c_str());
		}
		if (field == L"Wage")
		{
		...
		if (field == L"WorkDay")
		{
			worker->WorkDays[day] = _wtoi(value.c_str()) == 1;
			day++;
			if (day == 7)
			{
				day = 0;
		...
		{
			person->EmoticonTimer = (float)_wtoi(value.c_str());
		}
		if (field == L"Happiness")
		{
			person->Happiness = (float)_wtof(value.c_str());
		}
		if (field == L"Name")
		{
			person->Name = value;
		...
		{
			person->ShowEmotion = _wtoi(value.c_str()) == 1;
		}
		if (field == L"Speed")
		{
			person->Speed = (float)_wtof(value.c_str());
		}
		if (field == L"WaitTimer")
		{
			person->WaitTimer = (float)_wtof(value.c_str());
		}
		...
		{
			entity->AnimTimer = (float)_wtof(value.c_str());
		}
		if (field == L"DeleteMe")
		{
			entity->DeleteMe = _wtoi(value.c_str()) == 1;
		}
		if (field == L"GridPosX")
		{
			entity->GridPosition = sf::Vector2f((float)_wtof(value.c_str()), 0.0f);
		}
		if (field == L"GridPosY")
		{
			entity->GridPosition.y = (float)_wtof(value.c_str());
		}
		if (field == L"ID")
		{
		...
		{
			entity->LowestGridPosition = sf::Vector2f((float)_wtof(value.c_str()), 0.0f);
		}
		if (field == L"LowestGridPositionY")
		{
			entity->LowestGridPosition.y = (float)_wtof(value.c_str());
		}
		if (field == L"LowestScreenPositionX")
		{
			entity->LowestScreenPosition = sf::Vector2f((float)_wtof(value.c_str()), 0.0f);
		}
		if (field == L"LowestScreenPositionY")
		{
			entity->LowestScreenPosition.y = (float)_wtof(value.c_str());
		}
		if (field == L"Moved")
		{
			entity->Moved = _wtoi(value.c_str()) == 1;
		}
		...
		if (field.find(L"EntityProductName") != std::wstring::npos)
		{
			...
			if ((int)entity->Products.size() <= pos)
			{
		...
		{
			animal->WaitTimer = (float)_wtof(value.c_str());
		}
		if (field == L"Walking")
		{
			animal->Walking = _wtoi(value.c_str()) == 1;
		}
		if (field == L"ToBeDeleted")
		{
			animal->ToBeDeleted = _wtoi(value.c_str()) == 1;
		}
		if (field == L"ToBeDeletedAgain")
		{
			animal->ToBeDeletedAgain = _wtoi(value.c_str()) == 1;
		}
		if (field == L"LifeTime")
		{
			animal->LifeTime = (float)_wtof(value.c_str());
		}
	...
	while (iter != end)
	{
		...
		if (iter->ConnectionType == Connection::Type::Message)
		{
			...
			for (unsigned int i = 0; i < this->CurrentStore->Grid.size(); i++)
			{
				for (unsigned int j = 0; j < this->CurrentStore->Grid[i].size(); j++)
				{
