...
int main()
{
	...
	sf::View View(sf::Vector2f(0.0f, 500.0f), sf::Vector2f((float)window.getSize().x, (float)window.getSize().y));
	sf::View UIView(sf::FloatRect(0.0f, 0.0f, (float)window.getSize().x, (float)window.getSize().y));
	sf::FloatRect ResetRect(View.getCenter().x - View.getSize().x / 2.0f, View.getCenter().y - View.getSize().y / 2.0f, View.getSize().x, View.getSize().y);
	...
	while (window.isOpen())
	{
		...
		while (window.pollEvent(event))
		{
			...
			case sf::Event::Resized:
				View = sf::View(sf::FloatRect(0.0f, 0.0f, (float)event.size.width, (float)event.size.height));
				UIView = sf::View(sf::FloatRect(0.0f, 0.0f, (float)event.size.width, (float)event.size.height));
		...
		if (InputHandler.GetMousePos().x > (int)window.getSize().x - 20 || InputHandler.IsPressed(Input::ScrollRight))
		{
			View.move(500 * ElapsedTime.asSeconds(), 0);
			ResetRect.left += 500 * ElapsedTime.asSeconds();
		}
		...
		if (InputHandler.GetMousePos().y > (int)window.getSize().y - 20 || InputHandler.IsPressed(Input::ScrollDown))
		{
			View.move(0, 500 * ElapsedTime.asSeconds());
			ResetRect.top += 500 * ElapsedTime.asSeconds();
		}
			...
			if (UI.Messages.front().MessageType == InterfaceMessage::Type::ChangeGameState)
			{
				if (UI.Messages.front().Text == "BeforeOpening")
				{
					...
					if (TheGame.Data.Prestige >= 200)
					{
						...
						{
							for (unsigned int i = 0; i < TheGame.CurrentStore->PeepGenerator.ClassProbabilities.size(); i++)
							{
					...
					for (unsigned int i = 0; i < TheGame.IllegalStuff.size(); i++)
					{
