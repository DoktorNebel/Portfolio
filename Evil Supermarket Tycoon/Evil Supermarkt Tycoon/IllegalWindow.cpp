...
IllegalWindow::IllegalWindow(std::string name, sf::Sprite* sprite, sf::Vector2f position, TextureHandler* texHandler, std::vector<IllegalActivity>* illegalStuff)
{
	...
	for (unsigned int i = 0; i < illegalStuff->size() - 2; i++)
	{
		sstream.str("");
		sstream << i;
		this->Buttons.push_back(new InterfaceButton(sstream.str(), InterfaceButton::Type::Permanent, texHandler->GetSprite("CheckBox")[0][0], position, sf::Vector2f(10.0f, 10.0f + 50.0f * (float)i)));
	...
	this->HoverWindowSprite->setScale(0.6f, 0.1f);
...
void IllegalWindow::update(Input* input, GameData* gameData, std::list<InterfaceMessage>* messages)
{
	...
	while (iter != end)
	{
		...
		{
			mouseOver = true;
			this->HoverWindowSprite->setPosition((float)input->GetMousePos().x, (float)input->GetMousePos().y);
