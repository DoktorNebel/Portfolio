...
void Store::UpdateBackground()
{
	...
	for (int x = -40; x < this->GridSizeX + 40; x++)
	{
		for (int y = -40; y < this->GridSizeY + 40; y++)
		{
			...
			{
				float screenPosX = (float)(x - y) * (float)Constants::GridSize;
				float screenPosY = (float)(x + y) * (float)Constants::GridSize / 2.0f;
			...
			{
				float screenPosX = (float)(x - y) * (float)Constants::GridSize;
				float screenPosY = (float)(x + y) * (float)Constants::GridSize / 2;
...
void Store::update(sf::Time elapsedTime, unsigned short gameSpeed, GameData* gameData)
{
			...
			while (iter != end)
			{
				if (!processed[(*iter)->ID])
				{
					...
					if (object)
					{
						...
						if (this->Messages.size() > 0 && this->Messages.front().MessageType == StoreMessage::Type::RequestProductPos)
						{
							for (unsigned int k = 0; k < object->Slots.size(); k++)
							{
								if (object->Slots[k] && object->Slots[k]->Name == this->Messages.front().Name && object->Products[k].size() > 0)
								{
									...
									tmp.ID = this->Messages.front().ID;
									tmp.MessageType = StoreMessage::Type::SendPos;
									tmp.Position = sf::Vector2i((int)object->GridPosition.x + pos.x, (int)object->GridPosition.y + pos.y);
									tmp.Pointer = object;
									request = true;
								}
							}
					...
					if ((*iter)->Moved)
					{
						...
						{
							(*iter)->GridPosition.x = (float)this->Grid.size() - 1.0f;
						}
						...
						{
							(*iter)->GridPosition.y = (float)this->Grid[0].size() - 1.0f;
						}
						...
						{
							x = (int)std::ceil((*iter)->GridPosition.x);
						}
						else
						{
							x = (int)std::floor((*iter)->GridPosition.x);
						}
						if ((*iter)->GridPosition.y - (int)(*iter)->GridPosition.y >= 0.5f)
						{
							y = (int)std::ceil((*iter)->GridPosition.y);
						}
						else
						{
							y = (int)std::floor((*iter)->GridPosition.y);
						}
	...
	if (!this->DrugsDelivered && gameData->Time.Hour == this->DrugHour)
	{
		...
		tmp->GridPosition = sf::Vector2f((float)this->ExitPosition.x, (float)this->ExitPosition.y);
		tmp->LowestGridPosition = sf::Vector2f((float)this->ExitPosition.x, (float)this->ExitPosition.y);
	...
	if (this->SpawnTimer >= this->SpawnRate + (float)(rand() % 101) / 100.0f && this->Spawn)
	{
		...
		if (rand() % 100 < 5)
		{
			...
			animal->GridPosition = sf::Vector2f((float)this->ExitPosition.x, (float)this->ExitPosition.y);
			animal->LowestGridPosition = sf::Vector2f((float)this->ExitPosition.x, (float)this->ExitPosition.y);
...
void Store::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
	...
	for (int i = 0; i <= this->GridSizeX; i++)
	{
		float screenPosX = (float)(i - 0) * (float)Constants::GridSize;
		float screenPosY = (float)(i + 0) * (float)Constants::GridSize / 2.0f;
		line[0].position = sf::Vector2f(screenPosX, screenPosY);

		screenPosX = (float)(i - this->GridSizeY) * (float)Constants::GridSize;
		screenPosY = (float)(i + this->GridSizeY) * (float)Constants::GridSize / 2.0f;
	...
	for (int j = 0; j <= this->GridSizeY; j++)
	{
		float screenPosX = (float)(0 - j) * (float)Constants::GridSize;
		float screenPosY = (float)(0 + j) * (float)Constants::GridSize / 2.0f;
		line[0].position = sf::Vector2f(screenPosX, screenPosY);

		screenPosX = (float)(this->GridSizeX - j) * (float)Constants::GridSize;
		screenPosY = (float)(this->GridSizeX + j) * (float)Constants::GridSize / 2.0f;
		line[1].position = sf::Vector2f(screenPosX, screenPosY);
