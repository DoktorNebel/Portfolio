...
void EntityWindow::update(Input* input, GameData* gameData, std::list<InterfaceMessage>* messages)
{
	if (this->EntityChanged)
	{
		if (this->SelectedEntity != 0)
		{
			if (dynamic_cast<StoreObject*>(this->SelectedEntity))
			{
				...
				{
					this->OtherButtons[i] = TextButton("Price", TextButton::Type::Price, this->TexHandler->GetSprite("TextButton")[0][0], this->TexHandler, this->Sprite->getPosition(), sf::Vector2f((float)i * 150.0f + 30.0f, 140.0f));
					...
					this->OtherButtons[i + tmp.size()] = TextButton("ExpDate", TextButton::Type::ExpDate, this->TexHandler->GetSprite("TextButton")[0][0], this->TexHandler, this->Sprite->getPosition(), sf::Vector2f((float)i * 150.0f + 30.0f, 185.0f));
					...
					this->Texts[i].setPosition(this->Sprite->getPosition() + sf::Vector2f((float)i * 150.0f + 15.0f, 220.0f));
	...
	for (unsigned int i = 0; i < this->OtherButtons.size(); i++)
	{
	...
	if (input->JustClicked(sf::Mouse::Right) && dynamic_cast<StoreObject*>(this->SelectedEntity))
	{
		...
		{
			if (this->Rectangles[i].getGlobalBounds().contains((float)input->GetMousePos().x, (float)input->GetMousePos().y))
			{
...
void EntityWindow::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
	...
	if (this->SelectedEntity != 0)
	{
		for (unsigned int i = 0; i < this->ProductSprites.size(); i++)
		{
