...
void Entity::setColor(sf::Color color)
{
	...
	{
		for (unsigned int j = 0; j < this->Sprite[i].size(); j++)
		{
	...
	if (!this->BackSprite.empty())
	{
		...
		for (unsigned int i = 0; i < this->BackSprite.size(); i++)
		{
			for (unsigned int j = 0; j < this->BackSprite[i].size(); j++)
			{
...
void Entity::UpdateSprite(sf::Time elapsedTime, unsigned short gameSpeed)
{
	if (this->Sprite[0].size() > 1)
	{
		...
		if (AnimTimer >= 0.10f)
		{
			this->AnimTimer = 0.0f;
			this->AnimPhase++;
			if (this->AnimPhase >= (int)this->Sprite[this->AnimState].size())
			{
				this->AnimPhase = 0;
